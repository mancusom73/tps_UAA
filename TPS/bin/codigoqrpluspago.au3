...
Global $Titulo = "Plus Pago POS 1.51"
Global $blinkear = 0
Global $reintentos =0
...
GlobaL $ModoOffline = false; True  ;!!!!!!!esto debe estar False en producccion

if( $ModoOffline = True) Then
   Log2("estoy en modo offline ")
   MsgBox(48 + $MB_SYSTEMMODAL, "ERROR GRAVE", "ESTAS EN MODO OFFLINE  " & @CRLF &  "nO HAY CONSULTA AL SERVIDOR - Sale en 3 Seg", 3  )
EndIf
...
Global $sQrIni= @ScriptDir & "\qrpluspago.ini"

Local $NroOrden
Local $esproduccion = 0

$IpServidor = "10.9.0.1"
$PortServidor = "8091" ; este puesto es pluspago

#Region ### Codigos de repuesta de la aplicacion ###
; 2 no se autorizo  3 ok,  0 cerro  el script
Log2("Cantidad de parametros " & $CmdLine[0]  & @LF)
...
$Pic1 = GUICtrlCreatePic(@ScriptDir & "/pluspago.gif", 20, 208, 272, 128)
$Group1 = GUICtrlCreateGroup("Envio / Recepcion PlusPago", 400, 0, 289, 297)
...
if $levantado = 0 Then
   Log2("NO HAY CONEXION CON EL SERVICIO DE PLUS PAGO ")
   MsgBox(48 + $MB_SYSTEMMODAL, "ERROR GRAVE", "NO HAY CONEXION CON EL SERVICIO DE PLUS PAGO " & @CRLF &  "POR FAVOR AVISE AL SUPERVISOR - Sale en 3 Seg", 3  )
    if ($ModoOffline = False) Then
	     Exit(2)
	 EndIf
...
LeerNroOrden()
ActivarMensaje()
SetearTxt("PREPARANDO ENVIO" & @LF & @LF & @LF &"****ESPERE POR FAVOR****", $lblmensajes)
Sleep(1000)
;~ DesactivarMensaje()
...
Local $eliminada = BorrarAnteriores()
Local $accion = 2
...
;~     MsgBox($MB_SYSTEMMODAL, "", "Value of $i is: " & $i)
    ConsoleWrite2("Mando consulta de pago pluspago " & @LF)
...
;~ aca salio por timeout borrar la orden
		 MsgBox($MB_OK + $MB_ICONINFORMATION + $MB_SYSTEMMODAL, "MENSAJE PLUSPAGO","SE VENCIO EL TIEMPO DE ESPERA Sale en 3 Seg", 3  )
...
;~    aca salio porque encontro la merchar order
		  MsgBox($MB_OK + $MB_ICONINFORMATION + $MB_SYSTEMMODAL, "MENSAJE QR","SE REGISTRO EL PAGO EN PLUSPAGO MUCHAS GRACIAS Sale en 3 Seg", 3 )
;~ 		  ConsoleWrite("1 - se registro"& @LF)
		 ...
		 GuardarOrden()
	  Else
		  MsgBox($MB_OK + $MB_ICONINFORMATION + $MB_SYSTEMMODAL, "MENSAJE PAGO QR ","ERROR EN LA ORDEN O NO APROBADA- Sale en 3 Seg" , 3 )
;~ 		  ConsoleWrite("1 - se registro"& @LF)
		  ActivarMensaje()
...
;~ 	  en la devolucion de pruebas el referencia_orden lo saco del json hardcodeado
	  Local $valorkey = Json_ObjGet($Data2, "id_transaccion_interno")
...
;pregunto si quedo de mercadopago asi lo cambio por el formato de pluspago
Local $valorkey = Json_ObjExists($Data2, "referencia_orden")

if( $valorkey =0) Then
; pluspago y mercadopago usan distinto nombres pero el json.orden es el mismo
	$valorkey = Json_ObjExists($Data2, "id_transaccion_interno")
 Else
	Json_ObjDelete($Data2,"referencia_orden")
	ConsoleWrite2("Borro referencia_orden de mercadopago que no va "& @LF)
	Json_ObjPut($Data2, "id_transaccion_interno", $ordenaleatoria)
 endif
...
if( $valorkey ) Then
   ...
   $valorkey = Json_ObjGet($Data2,  "id_transaccion_interno")
   Json_ObjPut($Data2, "id_transaccion_interno", $ordenaleatoria)
   SetearTxt($ordenaleatoria, $txtnroorden)
...
Func CargarToken()
Local $hFile=FileOpen("AccessToken2.html",0)
If $hFile = -1 Then
	     FileClose($hFile)
		 ConsoleWrite2("No encontre token2 lo genero en blanco " & @CRLF)
		 FileWrite("AccessToken2.html", " ")
EndIf
...
;Set COM error handler
 ...
 $oHttp.Option($WinHttpRequestOption_SslErrorIgnoreFlags) = $WinHttpRequestOption_SslErrorIgnoreFlags_IgnoreAll
  ...
  With $oHttp
	   Switch $tipo_mensaje
	   Case 1 ; crear orden
		  	 .Open("POST", "https://" & $IpServidor &":" & $PortServidor &"/pay_orders/", False)
			 ;subir a 6
...
;~ 		   ConsoleWrite("Salgo del send con el siguiente codigo de error " & @error & " " & $micodigoerror & @CRLF)
		 ...
		 Case 2 ; consultar orden
			 ;http://127.0.0.1:8092/pay_orders/1?criterio_de_consulta=por_nro_caja&cod_sucursal=1
...
;~ 			$micodigoerror = @error
		 Case 3 ; borrar orden
			 ;http://127.0.0.1:8092/pay_orders/1?criterio_de_borrado=por_nro_caja&cod_sucursal=1
			.Open("DELETE", "https://" & $IpServidor &":" & $PortServidor &"/pay_orders/"& $NroCaja & "?criterio_de_borrado=por_nro_caja&cod_sucursal=" & $CodSucursal , False)
			.SetTimeouts("600000", "600000", "600000", "600000")
			.SetRequestHeader("Authorization", "Bearer " & $Bearer)
			.SetRequestHeader("Content-Type", "application/json")
			.Send()
...
;~ 			http://127.0.0.1:8091/merchant_orders/2
		;	Local $mercharorder = "https://" & $IpServidor &":" & $PortServidor &"/merchant_orders/" & GUICtrlRead($txtnroorden )
			Local $mercharorder =  "https://" & $IpServidor &":" & $PortServidor &"/transactions/"& GUICtrlRead($txtnroorden )  & "?criterio_de_consulta=por_id_transaccion_interno"
 			ConsoleWrite("el comando merchar order es  " & $mercharorder &@CRLF)
			...
			if ($ModoOffline = False) Then
			  .Send()
		   Else
			  ConsoleWrite("simulo que mando el 4  " & @CRLF)
			  Sleep(5000)
			EndIf
...
if( @error = -2147352567 OR ($ModoOffline = True And $tipo_mensaje = 1 )) Then ;fuerzo la salida por timeout para empezar a consultar
;~    salida por timeout
...
;~ recibi un 200 o similar por ende tenemos respuesta
 ...
 if ($ModoOffline = False) Then
   ...
   Else
	   if ($tipo_mensaje = 4) Then
		 $file = FileOpen("Received4.html", $FO_READ)
	  Else
		 $file = FileOpen("Received.html", $FO_READ)
	  EndIf

	  $sResponse = FileRead($file)
	  FileClose($file)
	  ; FORZAMOS LA RELECTURA
 EndIf
 ...
 if( $seguir <> 1 and  $seguir <> 0) Then
	Return $seguir
 EndIf
 if( $seguir = 1 and  $tipo_mensaje = 1 OR $tipo_mensaje = 4) Then
	Return $seguir
 EndIf
 ...
 if( $seguir =  0 and  ($tipo_mensaje = 5 or $tipo_mensaje = 6) and $Esdevolucion = 1 ) Then
	Return $seguir
 EndIf
 if( $seguir =  0 and  $tipo_mensaje = 1 ) Then ;pluspagos only
	Return 2
 EndIf
  if( $tipo_mensaje = 2 ) Then ;pluspagos only
	Return $seguir
 EndIf
 CargarDatos(False)
...
;~ 	  timeout
	  ...
	  Return 2
   Case 200
	  if($tipo_mensaje = 4 OR $tipo_mensaje = 6 OR $tipo_mensaje = 2) Then
;~ 		 este tipo de mensaje necesito saber si esta la orden o no
		 ConsoleWrite2("Chequear respuesta json merchar o id_pago o consulta de pagos" & @CRLF)
	    Return  LeerJsonCompuesto($tipo_mensaje)
;~ 	    Return 2
	 EndIf
	 if($tipo_mensaje = 3) Then
		 ConsoleWrite2("Vamos a ver si borro bien vino un 200" & @CRLF)
		 Return  LeerJsonCompuesto($tipo_mensaje)
	  EndIf
  Case  201
...
Func CargarDatos( $istoken)
    if( $istoken) Then
	  ...
	  Local $Json1 = FileRead(@ScriptDir & "\AccessToken2.html")
   Else
...
;~ 	Return ($Json2 = $Json3)
 ...
 ;ConsoleWrite2("2Test1 Result: " & $Json3 & @LF)
   ...
   $mp_status = functionOfMyDreams($Obj, "respuesta", "pp_http_status")
   if($tipo_mensaje <> 4) Then
	;  	  $mp_status = functionOfMyDreams($Obj, "respuesta", "pp_http_status")
		  ;Local $errorcito = functionOfMyDreams($Obj, "respuesta", "error")
		  Local $estado = functionOfMyDreams($Obj, "respuesta", "estado")
		  if ($tipo_mensaje <> 2 and StringCompare ( $mp_status , "400",0 ) = 0 ) Then
			 ConsoleWrite2("hay error reportado por servidor" & @LF)
			 $huboerror = 1
		  EndIf
		  if ( $tipo_mensaje <> 2 and $tipo_mensaje <> 3 and StringCompare ( $estado , "PENDIENTE",0 ) <> 0) Then
			 ConsoleWrite2("La orden no esta pendiente se encuentra en= "& $estado & @LF)
			 $huboerror = 1
		  EndIf
		  if ($tipo_mensaje = 2 ) then
			  if ( StringCompare ( $mp_status , "400",0 ) = 0  or StringCompare ( $mp_status , "404",0 ) = 0 ) Then
			   ConsoleWrite2("No hay pendientes" & @LF)
			   Return 0
			else
			    if ( StringCompare ( $mp_status , "200",0 ) = 0 ) Then
			        ConsoleWrite2("Hay pendientes para borrar y se debieron borrar" & @LF)
			         Return 1
				  EndIf
				  ConsoleWrite2("Mensaje return no encontrado" & @LF)
			EndIf
		 EndIf
		 if ($tipo_mensaje = 3 ) then
			 if ( StringCompare ( $mp_status , "400",0 ) = 0  ) Then
			   ConsoleWrite2("ERROR AL TRATAR BORRAR PENDIENTES" & @LF)
			   Return 0
			else
			   ConsoleWrite2("Borrado Exitoso" & @LF)
			   Return 1
			EndIf
		 EndIf
   ...
   Else
		;	$mp_status = "200"
	EndIf
    ConsoleWrite2("pp_http_status " & $mp_status & @TAB &  $tipo_mensaje & @LF)
...
if ($tipo_mensaje = 4) Then ;veamos si hay o Not
   ConsoleWrite2("busco estado transaccion en el pago" & @LF)
	;Local $importe_pagado = Json_Get($Obj, '[respuesta][ordenes][0][pagos][0][importe_transaccion]')
	Local $estado_transaccion = Json_Get($Obj, '[respuesta][estado_transaccion]')
	Local $pp_estado_orden_de_pago = Json_Get($Obj, '[respuesta][pp_estado_orden_de_pago]')
		 Local $posicionjson = 0;
			if ( $mp_status = 200 and StringCompare($estado_transaccion  , "REALIZADA", 0) = 0  and  StringCompare($pp_estado_orden_de_pago , "APROBADA",0) = 0 ) Then
				  SetearTxt($mp_status,$txtcodigorespuesta)
				  SetearTxt("pagada",$txtstatus)
				  ;ConsoleWrite2("parece que algo encontre" & @LF)
				  Local $importe_pagado = Json_Get($Obj, '[respuesta][monto]')
				  Local $nro_orden = Json_Get($Obj, '[respuesta][id_transaccion_interno]')
				  Local $transaccion_comercio_id = Json_Get($Obj, '[respuesta][transaccion_comercio_id]')
				  ...
				  EndIf
				  ConsoleWrite2("importe pagado "& $importe_pagado& @LF )
				  ConsoleWrite2("Estado transaccion "& $estado_transaccion& @LF )
				  ConsoleWrite2("Estado orden de pago "& $pp_estado_orden_de_pago& @LF )
				  ConsoleWrite2("Nro de Orden Recibida "& $nro_orden& " Nro de Orden enviada " & GUICtrlRead($txtnroorden ) & @LF )
				  Local $hFileOpen = FileOpen(@ScriptDir & "\trans_id.txt", $FO_OVERWRITE)
				  FileWrite($hFileOpen, $transaccion_comercio_id)
				  FileFlush($hFileOpen)
				  FileClose($hFileOpen)
				  Return 0

			else ; aca no hay pagado seguir o esta rechazada mmmm
				   ...
				   if( $EsDevolucion = 1) Then
					  ...
					  Return 1
				   EndIf
				   if( StringCompare($estado_transaccion  , "PENDIENTE", 0) = 0 ) Then
					    ConsoleWrite2("Sigue pendiente la orden de pago seguir esperando" & @LF)
						Return 2
					 EndIf
					if( StringCompare($estado_transaccion  , "RECHAZADA", 0) = 0  or StringCompare($estado_transaccion  , "EXPIRADA", 0) = 0 or   StringCompare($estado_transaccion  , "CANCELADA", 0) = 0) Then
					    ConsoleWrite2("NO HAY QUE SEGUIR PORQUE LA ORDEN ESTA " & $estado_transaccion & @LF)
						SetearTxt($estado_transaccion,$txtstatus)
						 ActivarMensaje()
						 GUICtrlSetData($lblmensajes,"ERROR!! ORDEN DE PAGO FUE " & $estado_transaccion)
						SetearTxt("ERROR!! ORDEN DE PAGO " & $estado_transaccion, $lblmensajes)
						MsgBox(48 + $MB_SYSTEMMODAL, "ERROR GRAVE", "ERROR!! LA ORDEN DE PAGO FUE " & $estado_transaccion  & @CRLF &  "PAGO NO VALIDO - Sale en 3 Seg", 3  )

						Local $nro_orden = Json_Get($Obj, '[respuesta][id_transaccion_interno]')
						Local $importe_pagado = Json_Get($Obj, '[respuesta][monto]')
						ConsoleWrite2("importe ORDEN "& $importe_pagado& @LF )
						ConsoleWrite2("Estado transaccion "& $estado_transaccion& @LF )
						ConsoleWrite2("Estado orden de pago "& $pp_estado_orden_de_pago& @LF )
						ConsoleWrite2("Nro de Orden Recibida "& $nro_orden& " Nro de Orden enviada " & GUICtrlRead($txtnroorden ) & @LF )
...
;~ 	  busquedmos los datos de instore order
	  ...
	  $res = StringRegExp($str, '(?:code|referencia_orden|importe|pp_http_status):([^\{,}]+)', 3)
	   if( @error = 0 ) Then
...
;~ en teoria si el open tiene false no hace falta esto
    ...
    $file = FileOpen(@ScriptDir & "\AccessToken2.html", 2) ; The value of 2 overwrites the file if it already exists
     FileWrite($file, $sResponse)
	 FileFlush($file)
     FileClose($file)
    CargarDatos( True)
...
$file = @ScriptDir & "\AccessToken2.html"
Local $hFile=FileOpen("AccessToken2.html",0)
Local $diatoken
Local $diadeltoken
...
If $hFile = -1 Then
		 ...
		 ConsoleWrite2("No encontre token2 lo genero en blanco " & @CRLF)
		 FileWrite("AccessToken2.html", " ")
		  $diatoken = FileGetTime($file, $FT_MODIFIED,  $FT_ARRAY)
		  $diatoken[1]= "01"
		  $diatoken[2] = "01"
   Else
	  $diatoken = FileGetTime($file, $FT_MODIFIED,  $FT_ARRAY)
EndIf
FileClose($hFile)
$diadeltoken = $diatoken[1]  &$diatoken[2]
...
Func BorrarAnteriores()
;http://127.0.0.1:8092/pay_orders/1?criterio_de_consulta=por_nro_caja&cod_sucursal=1
ConsoleWrite2("Borrando anteriores " & @LF)
Local $haypendientes = EnviarRequest(2)
if $haypendientes = 1 Then
   ConsoleWrite2("Hay pendientes " & @LF)
   EnviarRequest(3)
EndIf
EndFunc
