...
#if !defined(BTI_WIN) && !defined(BTI_OS2) && !defined(BTI_DOS) && !defined(BTI_NLM) && !defined(BTI_DOS_32R) && !defined(BTI_DOS_32P) && !defined(BTI_DOS_32B) && !defined(BTI_WIN_32) && !defined(BTI_OS2_32)
...
#if defined( BTI_DOS ) || defined( BTI_DOS_32R ) || defined( BTI_DOS_32P ) || defined( BTI_DOS_32B )
              ...
              BTI_WORD_PTR dataLength, BTI_VOID_PTR keyBuffer, BTI_SINT keyNumber )
{
    ...
    return BTRCALL( operation, posBlock, dataBuffer, dataLength, keyBuffer, keyLength, ckeynum );
}
#endif
...
BTI_API BTRVID( BTI_WORD operation, BTI_VOID_PTR posBlock, BTI_VOID_PTR dataBuffer,
                BTI_WORD_PTR dataLength, BTI_VOID_PTR keyBuffer, BTI_SINT keyNumber,
                BTI_BUFFER_PTR clientID )
{
    ...
    return BTRCALLID( operation, posBlock, dataBuffer, dataLength, keyBuffer, keyLength, ckeynum,
                      clientID );
}
#endif
              ...
              BTI_WORD_PTR dataLength, BTI_VOID_PTR keyBuffer, BTI_SINT keyNumber )
{
    ...
    status = BTRCALL( operation, posBlock, dataBuffer, &dataLen32, keyBuffer, keyLength, ckeynum );
                ...
                BTI_BUFFER_PTR clientID )
{
    ...
    status = BTRCALLID( operation, posBlock, dataBuffer, &dataLen32, keyBuffer, keyLength, ckeynum,
                        clientID );
                ...
                BTI_BUFFER_PTR clientID )
{
    ...
    if( ( stat == B_NO_ERROR ) && ( !btrieveVersionOkay ) ) {
        ...
        stat = BTRV( B_VERSION, posBlockx, &dataBufx, &dataLenx, keyBufx, keyNumx );
        ...
        if( stat == B_NO_ERROR ) {
            while( !done ) {
                ...
                switch( *( BTI_BYTE * )&dataBufx[typeOffset] ) {
                    ...
                    case 'N':
                        if( ( ( version == 6 ) && ( revision < 16 ) ) || ( version < 6 ) ) {
                ...
                BTI_BUFFER_PTR clientID )
{
    ...
    *( ( BTI_LONG_PTR )&( newParms.sign ) ) = *( ( BTI_LONG_PTR )"PMSW" );
    ...
    ( *_ImportedFunctions_ )[PROCESS7B].functionPtr( ( pmswParmBlock * )&newParms );
              ...
              BTI_WORD_PTR dataLength, BTI_VOID_PTR keyBuffer, BTI_SINT keyNumber )
{
    ...
    Make call to Btrieve.
    ======================================================================*/
    ( *_ImportedFunctions_ )[PROCESS7B].functionPtr( ( pmswParmBlock * )&XDATA );
...
BTI_VOID CallBTRV( BTI_ULONG );

BTI_SINT GetRealModeBuffer( FARPTR *, BTI_ULONG *, BTI_ULONG );

BTI_SINT SetUpBTRVData( FARPTR *, BTI_VOID_PTR, BTI_VOID_PTR, BTI_VOID_PTR );

BTI_VOID RetrieveBTRVData( FARPTR *, BTI_VOID_PTR, BTI_VOID_PTR, BTI_VOID_PTR );

BTI_SINT BlobInputDBlen( register XTRACTR * );

BTI_SINT ProcessIndirect( FARPTR *, BTI_VOID_PTR, BTI_WORD, BTI_SINT, BTI_WORD * );

BTI_SINT VerifyChunk( BTI_VOID_PTR, BTI_WORD, BTI_VOID_PTR, BTI_WORD, BTI_WORD, BTI_WORD );

/*
 * ** The following function are in assembly language- define with CDECL
 */

extern BTI_CHAR cdecl getLowMemByte( BTI_WORD, BTI_ULONG );

extern BTI_CHAR cdecl setLowMemByte( BTI_WORD, BTI_ULONG, BTI_BYTE );

extern BTI_WORD cdecl getLowMemWord( BTI_WORD, BTI_ULONG );

extern BTI_WORD cdecl setLowMemWord( BTI_WORD, BTI_ULONG, BTI_WORD );

extern BTI_ULONG cdecl getLowMemLong( BTI_WORD, BTI_ULONG );

extern BTI_ULONG cdecl setLowMemLong( BTI_WORD, BTI_ULONG, BTI_ULONG );

extern void cdecl copyToLow( unsigned short dst_sel, unsigned dst_offset, void *src,
                             unsigned int size );

extern void cdecl copyFromLow( void *dst, unsigned short src_sel, unsigned src_offset,
                               unsigned int size );
 ...
 * *****************************************************************************
 */
/* protected mode pointer to real memory */
BTI_SINT GetRealModeBuffer( FARPTR *protectedP, BTI_ULONG *realAddr, /* real mode pointer to real memory */
        ...
        #if defined( BTI_DOS_32B )
{
    ...
    inRegs.x.ebx = ( requestedSize + 15 ) / 16;
                ...
                BTI_BUFFER_PTR clientID )
{
    ...
    if( stat == B_NO_ERROR ) {
        ...
        if( stat == B_NO_ERROR ) {
            ...
            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.sign ), "PMSW", 4 );

            setLowMemLong( /* Clear flags */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.flags ), 0 );

            setLowMemLong( /* set function code to BTRVID code */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.functionCode ), BTRV_CODE );

            setLowMemLong( /* clear pmSwitchStatus */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.pmSwitchStatus ), 0 );

            setLowMemLong( /* set data length */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.dataLength ), sizeof( pmswParmBlock ) );

            setLowMemLong( /* set dataPtr to point to twoPointers structure */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, newParms.dataPtr ), realPtr + structoffset( RMBTRVID, twoPointers ) );

            setLowMemLong( /* set xDataPtr to Btrieve parameter block */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, twoPointers.xdataPtr ), realPtr + structoffset( RMBTRVID, btrv.XDATA ) );

            setLowMemLong( /* set pointer to clientID in low memory */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, twoPointers.clientIdPtr ), realPtr + structoffset( RMBTRVID, clientID ) );

            copyToLow( /* copy clientID from input parameter */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, clientID ), clientID, CLIENT_ID_SIZE );

            setLowMemLong( /* set data buffer pointer */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.DATA_BUF ), realPtr + structoffset( RMBTRVID, btrv.DATA_BUF ) );

            setLowMemWord( /* set data buffer length in parm block */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.BUF_LEN ), *dataLength );

            setLowMemLong( /* Position block */

            FP_SEL( protectedP ), FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.POS_BLOCK ), realPtr + structoffset( RMBTRVID, btrv.POS_BLOCK ) + 38 );

            setLowMemLong( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.FCB ),
                           realPtr + structoffset( RMBTRVID, btrv.POS_BLOCK ) );

            setLowMemWord( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.FUNCTION ),
                           operation );

            setLowMemLong( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.KEY_BUFFER ),
                           realPtr + structoffset( RMBTRVID, btrv.KEY_BUFFER ) );

            setLowMemByte( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.KEY_LENGTH ),
                           255 );
            setLowMemByte( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.KEY_NUMBER ),
                           keyNumber );

            setLowMemLong( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.STATUS ),
                           realPtr + structoffset( RMBTRVID, btrv.STATUS ) );

            setLowMemWord( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBTRVID, btrv.XDATA.XFACE_ID ),
                           VARIABLE_ID );
            tmpProtectedP = protectedP;

            FP_INCR( tmpProtectedP, structoffset( RMBTRVID, btrv ) );

            stat = SetUpBTRVData( &tmpProtectedP, posBlock, dataBuffer, keyBuffer );
            ...
            if( stat == B_NO_ERROR ) {
                ...
                *dataLength = getLowMemWord( FP_SEL( protectedP ),
                                             FP_OFF( protectedP )
                                           + structoffset( RMBTRVID, btrv.XDATA.BUF_LEN ) );
                stat = getLowMemLong( FP_SEL( protectedP ),
                                      FP_OFF( protectedP )
                ...
                RetrieveBTRVData( &tmpProtectedP, posBlock, dataBuffer, keyBuffer );
            }
        }
    }
              ...
              BTI_WORD_PTR dataLength, BTI_VOID_PTR keyBuffer, BTI_SINT keyNumber )
{
    ...
    if( stat == B_NO_ERROR ) {
        ...
        if( stat == B_NO_ERROR ) {
            ...
            setLowMemByte( FP_SEL( protectedP ),
                           FP_OFF( protectedP ) + structoffset( RMBUFF, XDATA.KEY_LENGTH ), 255 );
            ...
            if( stat == B_NO_ERROR ) {
                ...
                *dataLength = getLowMemWord( FP_SEL( protectedP ),
                ...
                stat = getLowMemLong( FP_SEL( protectedP ),
                                      FP_OFF( protectedP ) + structoffset( RMBUFF, STATUS ) );
...
BTI_SINT SetUpBTRVData( FARPTR *protectedP, BTI_VOID_PTR posBlock, BTI_VOID_PTR dataBuffer,
                        BTI_VOID_PTR keyBuffer )
{
    BTI_WORD function;
    ...
    function = getLowMemWord( FP_SEL( *protectedP ),
                              FP_OFF( *protectedP ) + structoffset( RMBUFF, XDATA.FUNCTION ) );

    function = function % S_WAIT_LOCK;

    copyLen = getLowMemWord( FP_SEL( *protectedP ),
                             FP_OFF( *protectedP ) + structoffset( RMBUFF, XDATA.BUF_LEN ) );

    if( function == B_GET_DIRECT ) {
        keyNumber = getLowMemByte( FP_SEL( *protectedP ),
                                   FP_OFF( *protectedP )
        ...
        if( keyNumber == ( BTI_CHAR )GET_DRTC_XTRACTOR_KEY ) {
            ...
            if( xtr->Signature & INDIRECT_BIT ) {
                bufLen = getLowMemWord( FP_SEL( *protectedP ),
                                        FP_OFF( *protectedP )
                                      + structoffset( RMBUFF, XDATA.BUF_LEN ) );
                /* application's data buffer */
                if( stat
                  = ProcessIndirect( protectedP, dataBuffer, bufLen, ( BTI_SINT )PREPROCESS_BLOBGET,
                                     &copyLen ) ) {
                    return ( stat );
                }
    ...
    if( function == B_CHUNK_UPDATE ) {
        ...
        if( xtr->Signature & INDIRECT_BIT ) {
            ...
            /* application's data buffer  */
            if( stat
              = ProcessIndirect( protectedP, dataBuffer, bufLen, PREPROCESS_BLOBPUT, &copyLen ) ) {
                return ( stat );
            }
    ...
    copyToLow( FP_SEL( *protectedP ), FP_OFF( *protectedP ) + structoffset( RMBUFF, POS_BLOCK ),
               posBlock, 128 );

    copyToLow( FP_SEL( *protectedP ), FP_OFF( *protectedP ) + structoffset( RMBUFF, KEY_BUFFER ),
               keyBuffer, 255 );

    copyToLow( FP_SEL( *protectedP ), FP_OFF( *protectedP ) + structoffset( RMBUFF, DATA_BUF ),
               dataBuffer, copyLen );
...
BTI_VOID RetrieveBTRVData( FARPTR *protectedP, BTI_VOID_PTR posBlock, BTI_VOID_PTR dataBuffer,
                           BTI_VOID_PTR keyBuffer )
{
    BTI_WORD function;
    ...
    keyNumber = getLowMemWord( FP_SEL( *protectedP ),
                               FP_OFF( *protectedP ) + structoffset( RMBUFF, XDATA.KEY_NUMBER ) );

    bufLen = getLowMemWord( FP_SEL( *protectedP ),
                            FP_OFF( *protectedP ) + structoffset( RMBUFF, XDATA.BUF_LEN ) );

    if( ( function == B_GET_DIRECT ) && ( keyNumber == ( BTI_CHAR )GET_DRTC_XTRACTOR_KEY )
     && ( ( ( GET_XTRACTR * )dataBuffer )->Signature & INDIRECT_BIT ) ) {
        BTI_WORD ignoredDataLen;

        ProcessIndirect( protectedP, dataBuffer, bufLen, POSTPROCESS_BLOBGET, &ignoredDataLen );
    }
    else {
        copyFromLow( dataBuffer, FP_SEL( *protectedP ),
                     FP_OFF( *protectedP ) + structoffset( RMBUFF, DATA_BUF ), bufLen );
    }
    copyFromLow( posBlock, FP_SEL( *protectedP ),
                 FP_OFF( *protectedP ) + structoffset( RMBUFF, POS_BLOCK ), 128 );

    copyFromLow( keyBuffer, FP_SEL( *protectedP ),
                 FP_OFF( *protectedP ) + structoffset( RMBUFF, KEY_BUFFER ), 255 );
}
...
BTI_SINT ProcessIndirect( FARPTR *protectedP, BTI_VOID_PTR usersDataBuf, BTI_WORD maxlen,
                          BTI_SINT Action, BTI_WORD *bytesDone )
{
    PUT_XTRACTR *xtr;
    ...
    *bytesDone = protectSize = ( BTI_WORD )BlobInputDBlen( ( XTRACTR * )xtr );
    ...
    switch( Action ) {
        case PREPROCESS_BLOBGET:
            ...
            */
            ...
            setLowMemLong( FP_SEL( reqB ), FP_OFF( reqB ), *( ( BTI_ULONG * )usersDataBuf ) );
        ...
        case PREPROCESS_BLOBPUT:
            ...
            copyToLow( FP_SEL( reqB ), FP_OFF( reqB ), xtr, protectSize );
            ...
            chunkFlag = getLowMemLong( FP_SEL( reqB ), FP_OFF( reqB ) );

            setLowMemLong( FP_SEL( reqB ), FP_OFF( reqB ), chunkFlag & ~INDIRECT_BIT );
    ...
    for( i = 0;i < limit;i++ )                  /* limit is the same for both types */ {
        ...
        stat = VerifyChunk( xtr, protectSize, chunkP, len, *bytesDone,
        ...
        if( Action != PREPROCESS_BLOBGET ) {
            switch( Action ) {
                case POSTPROCESS_BLOBGET:
                    copyFromLow( chunkP, FP_SEL( reqB ), FP_OFF( reqB ),
                                 min( len, maxlen - *bytesDone ) );

                    break;

                case PREPROCESS_BLOBPUT:
                    copyToLow( FP_SEL( reqB ), FP_OFF( reqB ), chunkP,
                               min( len, maxlen - *bytesDone ) );
...
BTI_SINT VerifyChunk( BTI_VOID_PTR userDB, BTI_WORD bytesToProtect, BTI_VOID_PTR ChunkP,
                      BTI_WORD ChunkLen, BTI_WORD bytesDone, BTI_WORD maxlen )
{
    register BTI_ULONG uNorm = Normalize( userDB ), cNorm = Normalize( ChunkP );
