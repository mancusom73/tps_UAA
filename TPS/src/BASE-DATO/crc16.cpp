...
/* variables */
unsigned int crc16tab[256] = {0x0000,0xC0C1,0xC181,0x0140,0xC301,0x03C0,0x0280,0xC241,0xC601,0x06C0,
...
unsigned int crcitttab[256] = {0x0000,0x1021,0x2042,0x3063,0x4084,0x50A5,0x60C6,0x70E7,0x8108,
...
unsigned short updcrc( unsigned short crc, short c )
{
    ...
    tmp = ( crc >> 8 ) ^ c;
    crc = ( crc << 8 ) ^ crcitttab[tmp];
    return crc;
}


/* update crc reverse */
unsigned short updcrcr( unsigned short crc, short c )
{
    int tmp;
...
unsigned short crc16( unsigned short crc, char *ptr, unsigned short len )
{
    unsigned int i;

    for( i = 0;i < len;i++ ) {
        crc = ( crc >> 8 ) ^ crc16tab[( crc ^ ptr[i] ) & 0xff];
    }
    return crc;
}
#endif
