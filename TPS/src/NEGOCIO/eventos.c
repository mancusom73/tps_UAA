		...
		/*if( !event_e_ticket.excluir_promocion
		 ...
		 if( event_e_ticket.cod_articulo == _TICKET_COD_ESPECIAL( h )
			...
			if( ( _TICKET_TIPO( h ) & _RESTA ) && ( _TICKET_ORIGEN( h ) == _ORIGEN_TICKET ) ) {
				//si se trata de un item de anulación, éste se debe excluir directamente
...
/****************************************************************************/
char aux[50];
	//	Los límites para los tipos de datos numéricos están definidos en la cabecera <limits>.
	memset(aux,0,sizeof(aux));
