...
static double rou( double d )
{
    return ROUND( d, ENTEROS, DECIMALES );
}
static double asig( double nombre, double valor )
{
    int index;
    index = ( int )floor( nombre );
    if( index < MAX_VAR ) {
        vars[index] = valor;
    }
    return valor;
}
static double var( double nombre )
{
    int index;
    index = ( int )floor( nombre );
    if( index < MAX_VAR ) {
...
#define TABLESIZE 17/*256*/
#define STD_LIB_NUM 13
static formu_item ftable[TABLESIZE] = {{
    "exp", exp,1,0},{"ln",  log,1,
    0},{"sin", sin,1,0},{"cos", cos,1,0},{"tan", tan,1,0},{"asin", asin,1,0},{
    "acos", acos,1,0},{"atan", atan,1,
    0},{"atan2",( Func )atan2,2,
    0},{"abs",  fabs,1,0},{"sqrt",  sqrt,1,0},{"pi", ( Func )pi,0,0},{
    "rnd", ( Func )rnd, 0, 1},
    /*returns a random number from 0 to 1 */
    {"round", ( Func )rou, 1, 0},
    /*redondea un numero a la cantidad de decimales definida por DECIMALES*/
    {"asignar", ( Func )asig, 2, 1},{"var", ( Func )var, 1, 1},{NULL,NULL,0}};
...
{
    formu_item *table_p;
    for( table_p = ftable;table_p->f != NULL && strcmp( name, table_p->name );table_p++ ) {
        ;
    }
...
static double value( formu func )
{
    ...
    for( ;; ) {
        switch( *function++ ) {
            case '\0':
                goto finish; /* there is a reason for this "goto":
            ...
            case '<':
                ...
                *bufp++ = result;
                break;
            case 'F':
                //DBG(printf("%s ",ftable[*function].name));
                switch( ftable[*function].n_pars ) {
                    case 0:
                        *bufp++ = ( ( Func0 )ftable[*function++].f )();
                        break;
                    case 1:
                        x = *--bufp;
                ...
                break;
            default:
                fset_error( "I1: unrecognizable operator" );
                return 0;
        }
    }
...
which takes place before the translator detects an error. */
{
    ...
    while( *scan ) {
        if( isin_real( *scan )
         || ( ( *scan == '+' || *scan == '-' ) && scan > source && *( scan - 1 ) == 'E' ) ) {
            numbers++;
            scan++;
            while( isin_real( *scan )
                || ( ( *scan == '+' || *scan == '-' ) && scan > source && *( scan - 1 ) == 'E' ) ) {
                scan++;
            }
        }
    ...
    return( numbers * num_size + operators * op_size + functions * num_size + variables * var_size
          + end_size );
...
formu translate( const char *sourc, const char *args, int *leng, int *error )
{
    UCHAR *result;
    char *source;
    ...
    if( !( function = ( UCHAR * )malloc( size_estim + 1 ) ) ) {
        /* out of memory */
        fset_error( "no memory" );
        *leng = 0;
        *error = -1;
    ...
    i_pctable = 0;   
    if( !( i_ctable = ( double* )calloc( Max_ctable, sizeof( double ) ) ) ) {
        /* out of memory */
        fset_error( "no memory" );
        free( function );
        *leng = 0;
    ...
    if( !result || fget_error() ) {
        ...
        *leng = result - function;
        ...
        has not been counted */ {
            ...
            nfunc = ( UCHAR * )calloc( ( ( *leng ) + 1 ), sizeof( UCHAR ) );
            if( nfunc ) {
        ...
        if( i_pctable < Max_ctable ) {
            ctable = ( double* )calloc( i_pctable, sizeof( double ) );
            if( ctable ) {
...
/* Only memory positions from *function to *comp_time are touched. */
{
    ...
    if( !( ( scan == fend - ( sizeof( ( UCHAR )'F' ) + sizeof( UCHAR ) ) && *( fend - 2 ) == 'F'
          && ftable[*( fend - 1 )].varying == 0 )
        || ( scan == fend - sizeof( UCHAR ) && is_code_oper( *( fend - 1 ) ) ) ) )
    ...
    *endf = tempch;
    ...
    if( ftable[n_function].n_pars == 0 ) {
        ...
        *( end - 1 ) = tempch;
        ...
        tempu = function;
        for( i = 0;i < ftable[n_function].n_pars;i++ ) {
            if( !( tempu = i_trans( tempu, paramstr[i], paramstr[i] + strlen( paramstr[i] ) ) ) ) {
...
static unsigned int r250_buffer[250] = {15301,57764,10921,56345,19316,43154,54727,49252,32360,49582,
                                        ...
                                        26899,25357,7667,3577,39414,51161,4,58427,57342,58557,53233,
                                        1066,29237,36808,19370,17493,37568,3,61468,38876,17586,
                                        64937,21716,56472,58160,44955,55221,63880,1,32200,62066,
                                        22911,24090,10438,40783,36364,14999,2489,43284,9898,39612,
                                        9245,593,34857,41054,30162,65497,53340,27209,45417,37497,
                                        4612,58397,52910,56313,62716,22377,40310,15190,34471,64005,
                                        18090,11326,50839,62901,59284,5580,15231,9467,13161,58500,
                                        7259,317,50968,2962,23006,32280,6994,18751,5148,52739,49370,
                                        51892,18552,52264,54031,2804,17360,1919,19639,2323,9448,
                                        43821,11022,45500,31509,49180,35598,38883,19754,987,11521,
                                        55494,38056,20664,2629,50986,31009,54043,59743};
...
void r250_init( int seed )
{
    ...
    mysrand( seed );
    r250_index = 0;
    for( j = 0;j < 50;j++ )     /* Fill the r250 buffer with 15-bit values */ {
        r250_buffer[j] = myrand();
    }
    for( j = 0;j < 50;j++ )     /* Set some of the MS bits to 1 */ {
        if( myrand() > 16384 ) {
...
double dr250( void )
{
    ...
    if( r250_index >= 250 )      /* Increment pointer for next time */ {
        r250_index = 0;
    }
    else {
        r250_index++;
