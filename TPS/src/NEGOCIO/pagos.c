...
/*****************************************************************************/
{
    ...
    if( _ESTADO == 7 ) {
		...
		agruparlas para merjorar la comprensión, existia un error an utilizar nota de credito y 
...
/*****************************************************************************/
{
    ...
    DestroyPanel( pPanel );
	/*------------- Controla restricciones de artï¿½culos --------------*/
	if( cod_restriccion_art != 0 ) {
		double sum, signo;
		int h;

		sum = 0.0;
		// sumo artï¿½culos que pueden ser pagados con este medio
   		...
   		// resto montos de pagos anteriores con el mismo cï¿½digo de restricciï¿½n
		for( h = 0; h < RAM_P_PAGO; h++ ) {
        ...
        _SET_MEMORY_CHAR( __pago_cuotas, pos, VAR_TARJETAS_CUOTAS );
{
        ...
        /*---------------------- Recalcula resto ---------------------*/
		/* Para Moneda extranjera, se analiza la versión, llegando a un acuerdo que es correcto el funcionamiento:
		el centavo/s faltantes surgen de la cotización y minima denominación de la moneda extranjera,
		es decir si el cliente paga con moneda extranjera, seguramente no pagara con denominaciones 
		pequeñas sino con valores cerrados , ej. u$s 100, 50, 20, 10,... muy rara vez intentará 
		completar los valores minimos exactos (ej.$15.22). La diferencia de 0.01 centavo de moneda 
		local, no es cubrible por la minima denominación posioble de la moneda extranjera de acuerdo
		con la cotización, por esta razon este excedente/faltante se solicita se pague con la moneda local.*/
...
/*****************************************************************************/
{
    ...
    for( ;GET_MEMORY_INT( _h ) < RAM_P_PAGO;ADD_MEMORY_INT( _h, 1 ) ) {
        ...
        SET_MEMORY_INT( __ram_ptr, h );//Se utiliza en impresión
...
//#endif
			...
			if (_rta == 1 && datos_medio[pos_actual].reingresar == 'S') {
				...
				ExecuteSSAction(A_PEDIR, _AJUSTABLE, titulo, cad_aux, NULL,
						tam_aux);
				//Posiciï¿½n actual para Reconstruir INPUT original.
				posicion = pos_actual;
				if (confirmar == -1) {//ESC para funciï¿½n CONFIRMAR_DATO_ADICIONAL
					/*Posiciï¿½n anterior para Reconstruir INPUT.
...
/******************************************************************************/
 ...
 //cod_valida = 2 validar compaï¿½ia o codigo de cobro
...
/***********************************************************************/
{
    ...
    if(pNuevo){ // si no es una eliminación de medios pago realizado
...
/***********************************************************************/
{
	...
	// Si ademas ya se encuentra en el último origen de restricciones se da por finalizada
	// la construcción del grafo y se calcula el flujo máximo sobre el mismo.
	if ( cant == grupo ) {
	...
	while ( pos <  cant ) {
		...
		if ( medios_requeridos.origen == origen && medios_requeridos.cod_grupo == grupo ) {
			...
			if ( medios_requeridos.monto_compartido ) {
				...
				// del medio requerido (notar que podría haber más de una ocurrencia)
				...
				// sólo uno de estos medios en cada grafo). A partir de cada uno de estos
				...
				// Entonces ahora, si agregando sólo esta arista al grafo, se encuentra un flujo que satisfaga
				// el requerimiento, se da por terminado el algoritmo. Sino, se seguirá probando
...
{
/*
 * Por el momento sólo se implementan las restricciones de medios con condicion CONDICION_MAYOR_IGUAL.
 * Las otras restricciones serán tratadas del mismo modo que las CONDICION_MAYOR_IGUAL.
 */
/*
 * Se construye un grafo con capacidades en los lados, sobre el cual se calculará
 ...
 */
	...
	int n = 2;	// cantidad de nodos del grafo. incialmente sólo la fuente y el resumidero.
	int m = 0;	// cantidad de aristas del grafo.
	int source_capacity = 0;	// Capacidad máxima de la fuente (para ser comparado luego con el flujo máximo)
	int j;
	int rta;
	char *buffer;
	int bufferInd = 0;
	...
	// Se recorren todos los pagos, agregando los nodos y aristas desde éstos hasta el resumidero,
	...
	for ( j = 0; j < RAM_P_PAGO; j++ ) {
		...
		buffer[bufferInd] = 0;
		n++; // indica que se agregó un nodo al grafo.
		m++; // indica que se agregó una arista al grafo.
	}
	ind += j; // actualiza el identificador del próximo nodo.
...
/*****************************************************************************/
{
   ...
   for( h = 0; h < RAM_P_PAGO; h++ ) {
      if( medios[_PAGO_MODO( h )].tarjeta ) {
         ...
         //Lo hago acá como para no buscar de nuevo este dato.
      }
	}
