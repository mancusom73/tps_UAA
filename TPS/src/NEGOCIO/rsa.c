...
static int RSAPublicBlock PROTO_LIST( ( unsigned char *, unsigned int *, unsigned char *,
                                        unsigned int, R_RSA_PUBLIC_KEY * ) );
static int RSAPrivateBlock PROTO_LIST( ( unsigned char *, unsigned int *, unsigned char *,
                                         unsigned int, R_RSA_PRIVATE_KEY * ) );

int _RSAPrivateDecrypt( char *buffer_out, int *len_out, char *buffer_in, int len_in,
...
R_RSA_PRIVATE_KEY *privateKey;                           /* RSA private key */
{
    NN_DIGIT c[MAX_NN_DIGITS],cP[MAX_NN_DIGITS],cQ[MAX_NN_DIGITS],dP[MAX_NN_DIGITS],
             dQ[MAX_NN_DIGITS],mP[MAX_NN_DIGITS],mQ[MAX_NN_DIGITS],n[MAX_NN_DIGITS],
             p[MAX_NN_DIGITS],q[MAX_NN_DIGITS],qInv[MAX_NN_DIGITS],t[MAX_NN_DIGITS];
...
/****************************************************************************/
{
   ...
   bytes_por_bloque = ( ( privateKey->bits) / 8 );
   ...
   *len_out         = 0;
   ...
   for( i = 0 ; i < cant_bloques && st == 0; i++ ) {
      if( i == cant_bloques - 1 )
   	...
   	st = RSAPrivateDecrypt( &buffer_out[off_out], &_len_out, &buffer_in[off_in], _len_in, privateKey );
      off_in 		+= _len_in;
      (*len_out) 	+= _len_out;
      off_out 		+= _len_out;
